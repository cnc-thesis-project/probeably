...
def start_server(jarm_sock="/tmp/jarm.sock", workers=10):
    ...
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM, 0) as sock:
        ...
        while True:
            ...
            try:
                ip, port = conn.recv(128).split(b" ")
                ip = ip.decode()
                port = int(port)
                hash = get_jarm(ip, int(port))
...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        jarm_sock = sys.argv[1]
        workers = int(sys.argv[2])
        start_server(jarm_sock, workers)
    else:
        print("Usage: {} <jarm-socket-path> <workerss>".format(sys.argv[0]))
